...
def setup_llamaindex_llm(llm_type: str = "mock", model_name: str = "llama3.2", 
                        api_key: Optional[str] = None, ollama_base_url: str = "http://localhost:11434") -> Optional['BaseLLM']:
...
def create_llamaindex_rag(chunks_df: DataFrame, llm: Optional['BaseLLM'], embed_model_name: str = "BAAI/bge-small-en-v1.5") -> Union['BaseQueryEngine', MockQueryEngine]:
...
def process_questions_with_rag(query_engine: Union['BaseQueryEngine', MockQueryEngine], questions: List[str]) -> List[Dict[str, Any]]:
