...
def get_evaluation_prompt(user_request: str, tool_selected: str, available_tools: str) -> str:
    """
    LEGACY: Direct evaluation prompt (not used in tutorial)

    This function is kept for backward compatibility but is not used
    in the main tutorial flow. The tutorial uses make_judge() instead.

    Args:
        user_request: The user's original request
        tool_selected: The tool that was selected by the agent
        available_tools: List of available tools (as string)

    Returns:
        Formatted evaluation prompt with JSON output format
    """
    return f"""You are an expert evaluator assessing AI agent tool selection.

User Request: {user_request}
Tool Selected by Agent: {tool_selected}
Available Tools: {available_tools}

Evaluate whether the agent's tool selection was appropriate.
Consider:
- Does the selected tool match the user's intent?
- Can this tool address the task requirements?
- Are there more suitable tools the agent overlooked?
...
{{
    ...
    "alternative_tool": "better option if applicable or null"
}}"""
